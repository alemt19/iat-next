...
export default function AdminResultsPage() {
    ...
    // Recargar resultados desde supabase
    ...
    const filteredResults = results.filter(r => {
        // Búsqueda texto plana por nombre, nivel, o descripción
        const plain = (
        r.d_score_level +
        ...
        const educationOk = filterEducation === "" || r.education === filterEducation
        return searchOk && genderOk && educationOk
    })
    ...
    const downloadCSV = () => {
        const csv = [
        [
            "ID", "D-Score", "Nivel", "Descripción", "Ensayos", "Rápidas", "RT Prom.", "Edad", "Género", "Educación", "Etnia", "Fecha"
        ].join(","),
    ...
    return (
        ...
        <Card className="w-full max-w-6xl mb-6">
            ...
            <div className="flex flex-wrap gap-4 mb-6 items-end">
                ...
                <input
                    ...
                    placeholder="Buscar por nivel, dscore, etapa, etc."
                ...
                <div>
                <label className="text-sm block mb-1">Educación</label>
                <select
                ...
                <Button variant="outline" onClick={fetchResults}>Refrescar</Button>
                <Button variant="default" onClick={downloadCSV}>Descargar CSV</Button>
            ...
            <div className="overflow-x-auto rounded border">
                ...
                <thead className="bg-blue-50">
                    ...
                    <th className="p-2 border font-medium">Educación</th>
                    <th className="p-2 border font-medium">Etnia</th>
                    ...
                    <th className="p-2 border font-medium text-center">Ver</th>
                    </tr>
                </thead>
                <tbody>
                    {filteredResults.length === 0 && (
                    <tr>
                        <td colSpan={9} className="py-8 text-center text-gray-500">No hay registros para mostrar.</td>
                    </tr>
                    )}
                    {filteredResults.map((r, i) => (
                    <tr
                        key={r.id}
                        className={`transition duration-75 ${i % 2 === 0 ? "bg-white" : "bg-gray-50"} ${selectedResult?.id === r.id ? "ring-2 ring-blue-300" : ""}`}
                    >
                        <td className="p-2 border">{r.id}</td>
                        <td className="p-2 border">{r.age}</td>
                        <td className="p-2 border">{r.gender}</td>
                        <td className="p-2 border">{r.education}</td>
                        <td className="p-2 border">{r.ethnicity}</td>
                ...
                {loading && (
                <div className="p-4 text-center text-blue-500 font-medium">
                    Cargando resultados...
                </div>
                )}
                {error && (
        ...
        {selectedResult && (
            <div className="fixed inset-0 z-30 flex justify-end bg-black/20" onClick={() => setSelectedResult(null)}>
            <div
            ...
            >
                ...
                <button className="text-2xl hover:text-red-500" onClick={() => setSelectedResult(null)}>&times;</button>
                </div>
                <div className="p-6 flex-1 space-y-5">
                <div><b>ID:</b> {selectedResult.id}</div>
                <div><b>Fecha:</b> {new Date(selectedResult.created_at).toLocaleString()}</div>
                <hr />
                <div><b>D-Score:</b> <span className="font-mono text-lg">{selectedResult.d_score.toFixed(3)}</span></div>
                <div><b>Nivel de preferencia:</b> {selectedResult.d_score_level}</div>
                <div><b>Interpretación:</b>
                    <div className="text-gray-700">{selectedResult.d_score_description}</div>
                </div>
                <hr />
                <div><b>Total de ensayos:</b> {selectedResult.total_trials}</div>
                <div><b>Respuestas muy rápidas (&lt;300ms):</b> {selectedResult.short_trials}</div>
                <div><b>RT promedio:</b> {Math.round(selectedResult.average_rt_ms)} ms</div>
                <hr />
                <div><b>Edad:</b> {selectedResult.age}</div>
                <div><b>Género:</b> {selectedResult.gender}</div>
                <div><b>Educación:</b> {selectedResult.education}</div>
                <div><b>Etnia:</b> {selectedResult.ethnicity}</div>
                </div>
